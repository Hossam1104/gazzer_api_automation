...
/**
 * Generates an Arabic address with a specific name for lookup.
 * Useful when you need to find the created address by name.
 *
 * @param name - Unique name identifier
 * @param workerIndex - Worker index (default: 0)
 * @returns Object with Arabic address data using specified name
 */
export function generateNamedArabicAddress(name: string, workerIndex: number | string = 0): any {
  const randomId = Math.floor(Math.random() * 1000);

  return {
    address: `عنوان-${name}`,
    street: "شارع التحرير",
    name: name,  // Use provided name for lookup
    building: `مبنى-${randomId}`,
    floor: "1",  // First floor (numeric string required by API)
    apartment: Math.floor(Math.random() * 500) + 1,
    lat: 27.164590 + (Math.random() * 0.02 - 0.01),
    long: 31.156531 + (Math.random() * 0.02 - 0.01),
    is_default: false
  };
}
...
/**
 * Common Arabic address field values for reference.
 * Useful for building custom test payloads.
 */
export const ArabicFieldValues = {
  addresses: [
    "العنوان الرئيسي",       // Main address
    "عنوان العمل",           // Work address
    "المنزل الثاني",         // Second home
    "عنوان التسليم",         // Delivery address
    "بيت العائلة"           // Family house
  ],
  streets: [
    "شارع التحرير",          // Tahrir Street
    "شارع النيل",            // Nile Street
    "شارع الجامعة",          // University Street
    "شارع الملك",            // King Street
    "شارع السلام"           // Peace Street
  ],
  buildings: [
    "مبنى رقم ٥",            // Building No. 5
    "مبنى التجارة",         // Commerce Building
    "مبنى الأسرة",          // Family building
    "برج النيل",            // Nile Tower
    "عمارة السلام"          // Peace Building
  ],
  floors: [
    "الأرضي",                // Ground floor
    "الطابق الأول",         // First floor
    "الطابق الثالث",        // Third floor
    "الطابق الخامس",        // Fifth floor
    "الطابق العاشر"         // Tenth floor
  ],
  names: [
    "المنزل",                // Home
    "المكتب",                // Office
